...
##
# Testing applications reside throughout the django_patterns source tree,
# close to the code which they are testing. In order for models defiend within
# these test applications to be created by the test runner, these apps need to
# be present in the INSTALLED_APPS. We'll take advantage of the new unittest
# discovery feature to find these testing applications in the source tree and
# add them to the end of INSTALLED_APPS.
#
# FIXME: Provide better documentation for this code.
#
# FIXME: Find a home for this code in the django_patterns, so that it can be
#        re-used by other Django applications.
import os
import sys
import subprocess
argv = [
